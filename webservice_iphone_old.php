...
//        $message      = "TripEnd";
  ...
  if($type=="DeclineTripRequest"){
	...
	if($type=="DateTest"){ 
		...
		echo print_r($returnArr,true);exit;
	}
	###########################################################################
	###########################################################################
				...
				if(count($row) > 0)
				{
					...
					if(count($res) > 0)
					{
						...
						if(count($vehicleTypes) > 0)
						{
							...
							for($i=0;$i<count($vehicleTypes);$i++){
                                $vehicleTypes[$i]['fPricePerKM'] = strval(round($vehicleTypes[$i]['fPricePerKM'] * $priceRatio,0));
                                $vehicleTypes[$i]['fPricePerMin']= strval(round($vehicleTypes[$i]['fPricePerMin'] * $priceRatio,0));
                                $vehicleTypes[$i]['iBaseFare']= strval(round($vehicleTypes[$i]['iBaseFare'] * $priceRatio,0));
                                $vehicleTypes[$i]['fCommision']= strval(round($vehicleTypes[$i]['fCommision'] * $priceRatio,0));
                                $vehicleTypes[$i]['iMinFare']= strval(round($vehicleTypes[$i]['iMinFare'] * $priceRatio,0));
                                $vehicleTypes[$i]['FareValue']= strval(round($vehicleTypes[$i]['fFixedFare'] * $priceRatio,0));
