...
if (isset($_POST['submit'])) {
    ...
    if ($iUserId == "" || $iUserId == "0" /* DISABLE BY SEYYED AMIR || $iDriverId == ""*/ || $iDriverId == "0"
        ...
        if ($iCabBookingId == "") {
            header("location:manual_dispatch.php?booking_id=" . $iCabBookingId . "&success=0&var_msg=" . $var_msg);
            exit;
        } else {
            header("location:manual_dispatch.php?booking_id=" . $iCabBookingId . "&success=0&var_msg=" . $var_msg);
            exit;
...
//$return5 = $generalobj->sendUserSMS($vPhone,$vcode,$message_layout,"");
    ...
    if ($return || $return1) {
        ...
        header("location:manual_dispatch.php?success=1&vassign=$edit&var_msg=$var_msg");
        die($var_msg);
    } else {
        $error = 1;
        $var_msg = $langage_lbl['LBL_ERROR_OCCURED'];
        header("location:manual_dispatch.php?success=0&vassign=$edit&var_msg=$var_msg");
        die($var_msg);
    }
