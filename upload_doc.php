...
$jdate = explode('/',$_POST['dLicenceExp']);
$mdate = jalali_to_gregorian($jdate[2],$jdate[1],$jdate[0]);

$_POST['dLicenceExp'] = $_REQUEST['dLicenceExp'] = $mdate[0] . '/' . $mdate[1] . '/' .$mdate[2] ;

#echo "<pre>";print_r($_REQUEST);print_r();exit;
...
if ($action == 'noc') {
    ...
    if($error_msg != '')
    {
        header("location:profile.php?success=0&id=".$_REQUEST['id']."&er=" . urlencode($error_msg));
        exit;
    }
    ///////////////////////////////
...
if ($action == 'certi') { 
    ...
    //print_r($_POST);
    //print_r($_FILES);
    //die("FILE SIZE: " . $_FILES['certi']['size']);
    // added by seyyed amir
    $error_msg = "";
...
if ($action == 'photo') {
    ...
    if($_FILES['photo']['size'] > $limit_file_size)
        $error_msg .= "files must be less than {$limit_kb}KB\r\n<br>";
    else
    {
	...
	if($image_name=="")
		{
			$var_msg = $langage_lbl['LBL_PLEASE_UPLOAD_VALID_IMAGE_FORMAT'];
			header("location:profile.php?success=0&id=".$_REQUEST['id']."&var_msg=" . $var_msg);
			//$generalobjAdmin->getPostForm($_POST, $var_msg, "company_document_action.php?success=0&id=".$_REQUEST['id']."&var_msg=".$var_msg);
         ...
         /*else if ($_FILES['photo']['size'] > 1048000) {
          ...
          if ($flag_error == 1) {
			...
			if($img1!=''){
               ...
               $vImage = $img1;
               $var_msg = $langage_lbl['LBL_PROFILE_IMAGE_UPLOADED'];
               if ($_SESSION['sess_user'] == 'driver') {
                    $tbl = 'register_driver';
                    $where = " WHERE `iDriverId` = '" . $_SESSION['sess_iUserId'] . "'";
               }
...
if(strtotime(date("Y-m-d")) > strtotime($_POST['dLicenceExp'])){
  $var_msg= $langage_lbl['LBL_EXPIRE_DATE_LOWER'];
  header("location:profile.php?success=0&id=".$_REQUEST['id']."&var_msg=" . $var_msg);
  exit;
}   
...
if($image_name=="" && $db_user[0]['dLicenceExp']!=$curdate)
{
  ...
  `iDriverId` = '" . $_SESSION['sess_iUserId'] . "'" . $where;
               $obj->sql_query($query);
  $var_msg=$langage_lbl['LBL_EXPIRE_DATE_UPDATED'];
