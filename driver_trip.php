...
//$sql = "SELECT d.vName,d.vLastName,sum(t.iFare),count(t.iDriverId) from register_driver d left join trips t on d.iDriverId = t.iDriverId where d.iDriverId = '29' "
 ...
 $sql = "SELECT t.*, u.vName, u.vLastName,t.tEndDate, t.iActive,d.vAvgRating, t.iFare, d.iDriverId,t.fRatioDriver,t.vCurrencyDriver, t.vRideNo, t.tSaddress,t.eType, d.vName AS name, d.vLastName AS lname,t.eCarType,t.iTripId,vt.vVehicleType_".$_SESSION['sess_lang']." as vVehicleType
    ...
    <!-- End: Left Menu-->
		...
		<div class="page-contant">
			<div class="page-contant-inner">
			  	<h2 class="header-page"><?php echo $langage_lbl['LBL_HEADER_TRIPS_TXT']; ?></h2>
		  		<!-- trips page -->
			  	<div class="trips-page">
			  		<form name="search" action="" method="post" onSubmit="return checkvalid()">
			  			<div class="Posted-date mytrip-page">
							<input type="hidden" name="action" value="search" />
				      		<h3><?php echo $langage_lbl['LBL_MYTRIP_SEARCH_RIDES_POSTED_BY_DATE']; ?></h3>
							<span>
                            <a onClick="return todayDate('dp4','dp5');"><?php echo $langage_lbl['LBL_MYTRIP_Today']; ?></a>
                            <a onClick="return yesterdayDate('dFDate','dTDate');"><?php echo $langage_lbl['LBL_MYTRIP_Yesterday']; ?></a>
                            <a onClick="return currentweekDate('dFDate','dTDate');"><?php echo $langage_lbl['LBL_MYTRIP_Current_Week']; ?></a>
                            <a onClick="return previousweekDate('dFDate','dTDate');"><?php echo $langage_lbl['LBL_MYTRIP_Previous_Week']; ?></a>
                            <a onClick="return currentmonthDate('dFDate','dTDate');"><?php echo $langage_lbl['LBL_MYTRIP_Current_Month']; ?></a>
                            <a onClick="return previousmonthDate('dFDate','dTDate');"><?php echo $langage_lbl['LBL_MYTRIP_Previous Month']; ?></a>
                            <a onClick="return currentyearDate('dFDate','dTDate');"><?php echo $langage_lbl['LBL_MYTRIP_Current_Year']; ?></a>
                            <a onClick="return previousyearDate('dFDate','dTDate');"><?php echo $langage_lbl['LBL_MYTRIP_Previous_Year']; ?></a>
				      		</span> 
				      		<span>
                            ...
                            <b><button class="driver-trip-btn"><?php echo $langage_lbl['LBL_Search']; ?></button>
								<button onclick="reset();" class="driver-trip-btn"><?php echo $langage_lbl['LBL_RESET']; ?></button></b> 
					      	</span>
				      	</div>
		      		</form>
			        		...
			        		<table width="100%" border="0" cellpadding="0" cellspacing="1" id="dataTables-example">
			          			<thead>
									...
									<?php if($APP_TYPE != 'UberX'){ ?> 
										...
										<?php } ?>
	        							<th width="17%"><?php echo $langage_lbl['LBL_RIDE_NO']; ?></th>
	        							<th width="18%"><?php echo $langage_lbl['LBL_DRIVER_TRIP_RIDER']; ?></th>
	        							<th width="15%"><?php echo $langage_lbl['LBL_DRIVER_TRIP_Trip_Date']; ?></th>
	        							<!-- <th width="6%">Rating</th> -->
	        							<th width="15%"><?php echo $langage_lbl['LBL_DRIVER_TRIP_FARE_TXT']; ?></th>
	        							<th width="15%"><?php echo $langage_lbl['LBL_DRIVER_TRIP_Car_Type']; ?></th>
	        							<th width="16%"><?php echo $langage_lbl['LBL_View_Invoice']; ?></th>
									</tr>
								</thead>
								<tbody>
								<?
								...
								<?php if($APP_TYPE != 'UberX'){ ?> 
									...
									<?php } ?>
									<td align="center" data-order="<?php echo $db_dtrip[$i]['iTripId']?>"><?php echo $db_dtrip[$i]['vRideNo'];?></td>
									
									<td ><?php echo $name;?></td>
									<td align="center">
										<?php
										if($db_dtrip[$i]['iActive'] != 'Canceled'){
											echo jdate('d-M-Y',strtotime($db_dtrip[$i]['tEndDate']));
										}else{
											echo "----";
										}
										?>
									...
									<td class="center">
									  <a href="invoice.php?iTripId=<?php echo base64_encode(base64_encode($db_dtrip[$i]['iTripId']))?>"><strong> <?php echo $langage_lbl['LBL_VIEW']; ?></strong></a>
									</td>
									<?php } ?>
								</tr>
							  	<?php } ?>
