...
@php
    if(is_null($vehicle)){
        $action = url('vehicles.create');
    }else{
        $action = url('vehicles.update',['id'=>$vehicle->iDriverVehicleId]);
    }
@endphp
                ...
                <select name="iCompanyId" id="iCompanyId"
                    ...
                    @foreach($companies as $company)
                        <option value="{{$company->iCompanyId}}"
                                {{(!is_null($vehicle) && $vehicle->iCompanyId== $company->iCompanyId) ? 'selected' : ''}}>{{$company->vCompany}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-12 col-md-6">
