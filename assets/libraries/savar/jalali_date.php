...
# change . to ،
define("CHANGE_POINT_TO_PERSIAN", false);
...
function jdate($format, $timestamp = NULL, $timezone = false, $fanum = NULL) {
    static $jdate_month_name = array('', 'فروردین', 'اردیبهشت', 'خرداد', 'تیر', 'مرداد', 'شهریور', 'مهر', 'آبان', 'آذر', 'دی', 'بهمن', 'اسفند');
...
function savar_request_date_to_gregorian($date)
{
	...
	$date_array = jalali_to_gregorian($date_array[0],$date_array[1],$date_array[2]);
	
    if(strlen($date_array[1]) == 1)
        $date_array[1] = '0' . $date_array[1];
    if(strlen($date_array[2]) == 1)
...
function savar_mysql_date_to_shamsi($date)
{
	$dateTime = strtotime($date);
	
	return jdate("Y-m-d H:i:s",$dateTime);
}