...
class General
{
    ...
    function get_user_available_balance_date($sess_iMemberId, $type, $date)
    {
        global $obj;

        $balance = 0;
        $sql = "SELECT SUM(iBalance) as totcredit FROM user_wallet WHERE iUserId = '" . $sess_iMemberId . "' AND eUserType = '" . $type . "' AND eType = 'Credit' AND " . $date;
        $db_credit_balance = $obj->MySQLSelect($sql);

        $sql = "SELECT SUM(iBalance) as totdebit FROM user_wallet WHERE iUserId = '" . $sess_iMemberId . "' AND eUserType = '" . $type . "' AND eType = 'Debit' AND " . $date;
    ...
    function send_email_user($type, $db_rec = '', $newsid = '', $lang = "PS")
    {
        ...
        // $maillanguage = (isset($maillanguage) && $maillanguage != '') ? $maillanguage : 'PS';
        $maillanguage = (isset($lang) && $lang != '') ? $lang : 'PS';

        $mailsubject = $res[0]['vSubject_' . $maillanguage];
        $tMessage = $res[0]['vBody_' . $maillanguage];
        $tMessage = str_replace($key_arr, $val_arr, $tMessage);
        $tMessage = $this->general_mail_format_html($tMessage, $maillanguage);
    ...
    {
        global $generalobj;
        require_once(TPATH_CLASS . 'savar/class.payamakpardaz.php');
        ...
        //$payamakp = new PayamPardaz($account_sid, $auth_token, $MobileNum);

        // بعد از اینکه قرار شد تنظیمات از پنل ادمین گرفته شود
        // خط بعد پاک شود
        $payamakp = new PayamPardaz();

        $mobileNo = $this->clearPhone($mobileNo);

        $toMobileNum = "+" . $code . $mobileNo;

        $ret = $payamakp->SendSMS($toMobileNum, $fpass);

        return $ret ? '1' : '0';
    }

    function general_mail_format_html($mail_body, $dir_from = "")
    {
        ...
        $mail_str = "";
        $mail_str = '
<div dir=' . $dir . ' style="width:100%!important;margin-top:0;margin-bottom:0;margin-right:0;margin-left:0;padding-top:0;padding-bottom:0;padding-right:0;padding-left:0;font-family:Arial, Helvetica, sans-serif;overflow-x:auto">
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    function Make_Price($text, $parameter = 0)
    {
        return number_format($text, $parameter, '.', ',');
    }

    function Make_Currency($text, $parameter = 0, $defCurrency = "")
    {
    ...
    function booking_currency($text, $defCurrency = "", $parameter = 0)
    {
    ...
    function clean($str)
    {
        ...
        if (!is_array($str)) { // should not be array only string will be clean
            $str = trim($str);
            $str = mysqli_real_escape_string($condbc, $str);
        }
    ...
    function getConfigurations($tableName, $LABEL)
    {
        global $obj;

        $sql = "SELECT vValue FROM `" . $tableName . "` WHERE vName='$LABEL'";
        $Data = $obj->MySQLSelect($sql);
    ...
    function trip_currency($price = '', $ratio = '', $defCurrency = "", $parameter = 0)
    {
        global $obj;
        if ($defCurrency == '') {
            $ssql = " eDefault='Yes'";
        ...
        if (count($db_curr_mst) > 0) {
            if ($ratio == '' || $ratio == 0) {
                return number_format($price, $parameter, '.', '') . ' ' . $db_curr_mst[0]['vSymbol'];
                //return number_format($price, $parameter, '.', '').' '.$db_curr_mst[0]['vSymbol'];
            } else {
                return number_format(($price * $ratio), $parameter, '.', '') . ' ' . $db_curr_mst[0]['vSymbol'];
                //return number_format(($price*$ratio), $parameter, '.', '').' '.$db_curr_mst[0]['vSymbol'];
            }
        }
    }
    ...
    function trip_currency_payment($price = '', $ratio = '', $defCurrency = "", $parameter = 0)
    {
        global $obj;
        #echo $ratio;exit;
        if ($defCurrency == '') {
    ...
    function trip_price($price = '', $ratio = '', $parameter = 0)
    {
        if ($ratio == '') {
            return number_format($price, $parameter, '.', '');
        } else {
    ...
    function sendCode($mobileNo, $code, $fpass = 'code', $pass = '')
    {
        ...
        $ret = $payamakp->SendSMS($toMobileNum, $verificationCode);
    ...
    // edit by seyyed amir
    function ganaraterefercode($ereftype, $fix = '')
    {

        global $obj;

        $charArray = "asdfghjkqwertyuipzxcvbnm"; // without l,o
        $charArrayLen = strlen($charArray);
        ...
        if ($fix == '') {
            ...
            $maxRes = $obj->MySQLSelect($sql);


            if (count($maxRes) > 0 && intval($maxRes[0]['maxid']) != 0) {
                $fixNum = intval($maxRes[0]['maxid']) + 1;
            } else
                $fixNum = rand(100, 999);
        } else
            $fixNum = $fix;
        ...
        do {
            $refCode = $prefix
                //. $charArray[rand(0, $charArrayLen- 1)]
                . rand(0, 9)
                . $fixNum;
            $refCode .= rand(10, 99);


        } while ($this->CheckReferralCodeExists($refCode) == true);

        return $refCode;
    }

    function CheckReferralCodeExists($refCode)
    {
        global $obj;
        $sql = "SELECT iDriverId FROM `register_driver` WHERE `vRefCode` = '$refCode'";
        $res = $obj->MySQLSelect($sql);

        if (count($res) > 0)
            return true;

        $sql = "SELECT iUserId   FROM `register_user`   WHERE `vRefCode` = '$refCode'";
        $res = $obj->MySQLSelect($sql);
    ...
    function InsertIntoUserWallet($iUserId, $eUserType, $iBalance, $eType, $iTripId, $eFor, $tDescription, $ePaymentStatus, $dDate, $referenceId)
    {

        global $obj;

        $eUserTypei = $eUserType;
        if ($eUserType == "driver") {
    ...
    function userwalletcurrency($currencyratio, $amount, $currencysymbol)
    {
        ...
        if ($amt == 0) {
            $finalamt = $db_currency[0]['vSymbol'] . " 0";
            // added by seyyed amir for iranian cSymbol
            $finalamt = "0 " . $db_currency[0]['vSymbol'];
            return $finalamt;
        } else {
            $finalamt = $db_currency[0]['vSymbol'] . ' ' . number_format($amt, $parameter, '.', '');
            // added by seyyed amir for iranian cSymbol
            $finalamt = number_format($amt, $parameter, '.', '') . ' ' . $db_currency[0]['vSymbol'];
            return $finalamt;
        }
    }
