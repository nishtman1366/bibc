...
class DBConnection
{
    ...
    public function MySQLQueryPerform($table, $data, $action = 'insert', $parameters = '')
    {
        ...
        if ($action == 'insert') {
            ...
            $results = @mysqli_query($conn,$query) or die("Query failed: " . mysqli_error($conn));
            ...
            #if(function_exists("TLOG")) TLOG($query);
            $results = @mysqli_query($conn,$query) or die("Query failed: " . mysqli_error($conn));
            if (!$results) {
                $this->MySQLDie("Query went bad!");
                return false;
    ...
    /*public function Getfieldtype($table,$field)
    {
        ...
        $results = mysqli_query($conn,$sql) or die(mysqli_error($conn)."query fail");
