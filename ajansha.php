    ...
    /////////////////////
     ...
     if ($image_name != "") {
         ...
         /* else if ($_FILES['insurance']['size'] > 1048576) {
          ...
          if ($flag_error == 1) {
               $generalobj->getPostForm($_POST, $var_msg,"ajans?success=0&var_msg=" . $var_msg);
               exit;
          } else {
     ...
     if ($image_name1 != "") {
         ...
         /*  if ($check_file['vPermit'] != '' && file_exists($check_file['vPermit'])) {
          ...
          if ($flag_error == 1) {
               $generalobj->getPostForm($_POST, $var_msg, "ajans?success=0");
               exit;
          } else {
               // $Photo_Gallery_folder = $tconfig["tsite_upload_vehicle_doc"];
               $Photo_Gallery_folder = $tconfig["tsite_upload_vehicle_doc"]. '/' . $iVehicleId . '/';
...
*/
     if ($image_name2 != "") {
         ...
         /* if ($check_file['vRegisteration'] != '' && file_exists($check_file['vRegisteration'])) {
          ...
          if ($flag_error == 1) {
               $generalobj->getPostForm($_POST, $var_msg, $tconfig['tsite_url'] . "ajans?success=0");
               exit;
          } else {
               // $Photo_Gallery_folder = $tconfig["tsite_upload_vehicle_doc"];
                $Photo_Gallery_folder = $tconfig["tsite_upload_vehicle_doc"]. '/' . $iVehicleId . '/';
...
if ($action == 'panel') {
  $resultdb = mysqli_query($condbc,"select count(*) from company where iParentId = '" . $_SESSION['sess_iUserId'] . "' and `iCompanyId` = '" . $id . "'");

                                              while($rowdb = mysqli_fetch_array($resultdb))
                                                {
                                                $result['count'] = $rowdb['count(*)'];
                                              }


                                              $resultdb2 = mysqli_query($condbc,"select * from company where iParentId = '" . $_SESSION['sess_iUserId'] . "' and `iCompanyId` = '" . $id . "'");
                                              ...
                                              if($result['count'] == 1)
                                              {
                                                ...
                                                $_SESSION['parent'] = $_SESSION['sess_iUserId'];
                                                ...
                                                header("Location:ajans?error=1&var_msg=مشکلی پیش آمده");
                                              }
}
...
if ($action == 'delete') {
  $resultdb = mysqli_query($condbc,"select * from company where iParentId = '" . $_SESSION['sess_iUserId'] . "'");

                                              while($rowdb = mysqli_fetch_array($resultdb))
                                                {
                                                $result['Company'] = $rowdb['vCompany'];
                                                $result['Email'] = $rowdb['vEmail'];
                                                $result['Phone'] = $rowdb['vPhone'];
                                                //$result['sender'] = $rowdb['sender'];
                                                $result['Address'] = $rowdb['vCaddress'];
                                                $result['City'] = $rowdb['vCity'];
                                                $result['Id'] = $rowdb['iCompanyId'];
                                                $result['eStatus'] = $rowdb['eStatus'];
     ...
     foreach ($db_driver_vehicle as $val) {
          ...
          $sql = "select count(*) from company where iCompanyId = '" . $id . "'";
          $db_usr = $obj->MySQLSelect($sql);
          ...
          if (count($db_usr) > 0 && $db_usr['trip_cnt'] > 0) {
               ...
               $query = "UPDATE `company` SET eStatus = 'Inactive' WHERE iCompanyId = '" . $id . "'";
               $obj->sql_query($query);

               header("Location:ajans?success=1&var_msg=با موفقیت غیر فعال شد");
               exit;
          }
}
else {
  ...
  if (count($db_usr) > 0 && $db_usr['trip_cnt'] > 0) {
       ...
       $query = "UPDATE `company` SET eStatus = 'Active' WHERE iCompanyId = '" . $id . "'";
       $obj->sql_query($query);

       header("Location:ajans?success=1&var_msg=با موفقیت فعال شد");
       exit;
}
}
}
    ...
    <!-- End: Left Menu-->
        ...
        <div class="page-contant">
            ...
            <div class="page-contant-inner">
							<center>
							<form action="ajans" method="post">
								<!--<input type="text" name="serch" placeholder="جست و جو">-->

								<input type="text" class="form-control" name="serch" id="serch" value="" placeholder="جست و جو" style="width: 100%;">
<label for="serch"><input type="submit" value="جست و جو" class="btn btn-sm btn-info" style="margin-top: 7px;"></label>
							</form>
						</center>
                ...
                <div class="vehicles-page">
                    <div class="accordion">
                        <?php
												...
												if($serch != '')
												{
													$resultdb = mysqli_query($condbc,"select * from company where iParentId = '" . $_SESSION['sess_iUserId'] . "' and (`vCompany` LIKE '%".$serch."%')");
												}
												else {
													$resultdb = mysqli_query($condbc,"select * from company where iParentId = '" . $_SESSION['sess_iUserId'] . "'");
												}
                        ...
                        <input type="hidden" name="iVehicleId" value = "<?php echo $db_driver_vehicle[$i]['iDriverVehicleId']; ?>"/>
                            <div class="accordion-section">
                                <div class="accordionheading">
                                   <?php if($APP_TYPE == 'UberX'){
                                    ...
                                    ?>
                                    <h3><?php echo $result['Company']  ;?> <br> <?php echo $result['Phone']  ;?></h3>
                                    <h4 style="margin:20px;float:right"><?php echo  $result['Address']  ;?> <br> <?php echo $result['Email']  ;?></h4>
                                    <span>
                                        <b>

                                          <?php
                                          $class_name = ($result['eStatus'] == "Active")? 'badge success-vehicle-active': 'badge success-vehicle-inactive';
                                          ?>

                                      <span class="<?php echo $class_name; ?>">
                                         <i class="<?php echo  ($result['eStatus'] == "Active") ? 'icon-eye-open' : 'icon-eye-close' ?>"></i>
												<?php
													if($result['eStatus'] == "Active"){
														echo $langage_lbl['LBL_ACTIVE_TEXT'];
													}else{
														echo $langage_lbl['LBL_INACTIVE_TEXT'];
													}
												?>
											 </span>
                                            <a href ="ajans?action=panel&id=<?php echo  $result['Id'] ?>" class="active">دسترسی به پنل</a>
                                            <?php if($APP_TYPE != 'UberX'){?>
                                            <a class="active active2" onClick="confirm_delete('<?php echo  $result['Id'] ?>');" href="javascript:void(0);"><?php echo  ($result['eStatus'] == "Active") ? 'غیر فعال' : 'فعال' ?></a><?php } ?>
                                ...
                                <div id="accordion-<?php echo $i;?>" class="accordion-section-content">
                                    <div class="driver-vehicles-page-new">
                                        ...
                                        <div class="alert alert-warning alert-dismissable">
                                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><?php echo $langage_lbl['LBL_PLESE_UPLOAD_INSURENCE_DOCUMENT']; ?> <!-- <a href="#" class="alert-link">Alert Link</a> -->.
                                        </div>
                                        <?php } ?>
                                        <?php if($db_driver_vehicle[$i]['vPermit'] == ''){?>
                                        <div class="alert alert-warning alert-dismissable">
                                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><?php echo $langage_lbl['LBL_PLESE_UPLOAD_PERMIT_DOCUMENTS']; ?><!-- <a href="#" class="alert-link">Alert Link</a> -->.
                                        </div>
                                        <?php } ?>
                                        <?php if($db_driver_vehicle[$i]['vRegisteration'] == ''){?>
                                        <div class="alert alert-warning alert-dismissable">
                                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><?php echo $langage_lbl['LBL_PLESE_UPLODEREGISTRATION_DOCUMENTS']; ?><!-- <a href="#" class="alert-link">Alert Link</a> -->.
                                        </div>
                                        <?php  }
                                          // }
                                         ?>
                                            ...
                                            <li>
                                                ...
                                                <div class="fileupload fileupload-new" data-provides="fileupload">
                                                    <div class="fileupload-preview thumbnail" style="width: 200px; height: 150px; ">
                                                        ...
                                                        ?>
                                    <img src = "<?php echo  $tconfig["tsite_upload_vehicle_doc_panel"] . '/' . $db_driver_vehicle[$i]['iDriverVehicleId']. '/' .$db_driver_vehicle[$i]['vInsurance'] ?>" style="width:200px;" alt ="Insurance Image"/>
                                                        <?php
                                                                }else{
                                                        ?>
                                                            <a href="<?php echo  $tconfig["tsite_upload_vehicle_doc_panel"] . '/' . $db_driver_vehicle[$i]['iDriverVehicleId']. '/' .$db_driver_vehicle[$i]['vInsurance']  ?>" target="_blank"><?php echo $langage_lbl['LBL_INSURENCE_DOC']; ?></a>
                                                        <?php
                                                                }
                                                            }
                                                            else {
							...
							<!--	<a href="#" class="btn btn-danger" onClick="return del_veh_doc('<?php echo $db_driver_vehicle[$i]['iDriverVehicleId']?>','vPermit','<?php echo $db_driver_vehicle[$i]['vPermit']?>');">X</a> -->
                              <img src = "<?php echo  $tconfig["tsite_upload_vehicle_doc_panel"] . '/'.$db_driver_vehicle[$i]['iDriverVehicleId']. '/'. $db_driver_vehicle[$i]['vPermit'] ?>" style="width:200px;" alt ="Insurance Image"/>
                                                        <?php
                                                                }else{
                                                        ?>
                                                            <a href="<?php echo  $tconfig["tsite_upload_vehicle_doc_panel"] . '/' .$db_driver_vehicle[$i]['iDriverVehicleId']. '/'. $db_driver_vehicle[$i]['vPermit']  ?>" target="_blank"><?php echo $langage_lbl['LBL_PERMIT_DOC']; ?></a>
                                                        <?php
                                                                }
                                                            }
                                                            else {
                                                    ...
                                                    <div class="fileupload-preview thumbnail" style="width: 200px; height: 150px; ">
                                                        ...
                                                        <?php
                                                            if($db_driver_vehicle[$i]['vRegisteration'] != ''){
                                                                ...
                                                                if($file_ext == 'is_image'){
                                                        ?>
                       <img src = "<?php echo  $tconfig["tsite_upload_vehicle_doc_panel"] . '/' .$db_driver_vehicle[$i]['iDriverVehicleId']. '/'.$db_driver_vehicle[$i]['vRegisteration'] ?>" style="width:200px;" alt ="Insurance Image"/>
                                                        <?php
                                                                }else{
                                                        ?>
                                                            <a href="<?php echo  $tconfig["tsite_upload_vehicle_doc_panel"] . '/' .$db_driver_vehicle[$i]['iDriverVehicleId']. '/'. $db_driver_vehicle[$i]['vRegisteration']  ?>" target="_blank"><?php echo $langage_lbl['LBL_INSURENCE_DOC']; ?></a>
                                                        <?php
                                                                }
                                                            }
                                                            else {
                                        ...
                                        <abbr><input type="submit" name="Submit" class="save-document" value="<?php echo $langage_lbl['LBL_Save_Documents']; ?>"></abbr>
                                    </div>
                                </div>
...
<?php
    $resultdb = mysqli_query($condbc,"select * from company where iCompanyId = '" . $id . "'");
                                                ...
                                                if($result['eStatus'] == "Active")
                                                {
                                                  $msgdeleted = "آیا میخواهید پنل آژانس مورد نظر را غیر فعال کنید؟";
                                                }
                                                else {
                                                  $msgdeleted = "آیا میخواهید پنل آژانس مورد نظر را فعال کنید؟";
                                                }
                                                ?>
        ...
        function confirm_delete(id)
        {
            ...
            if (id != '') {
                 var confirm_ans = confirm("<?php echo $msgdeleted;?>");
                 if (confirm_ans == true) {
                      window.location.href = "ajans?action=delete&id="+id;
                 }
                 }
